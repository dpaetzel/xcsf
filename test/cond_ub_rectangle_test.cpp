...
extern "C" {
#include "../xcsf/cl.h"
#include "../xcsf/cond_ub_rectangle.h"
...
TEST_CASE("COND_UB_RECTANGLE")
{
    struct XCSF xcsf;
    struct Cl c1;
    rand_init();
    param_init(&xcsf, 5, 1, 1);
    cond_param_set_type(&xcsf, COND_TYPE_UB_HYPERRECTANGLE);
    cond_param_set_min(&xcsf, 0);
    cond_param_set_max(&xcsf, 1);
    cond_param_set_spread_min(&xcsf, 1);
    cl_init(&xcsf, &c1, 1, 1);
    cond_ub_rectangle_init(&xcsf, &c1);
    ...
    struct CondUBRectangle *p = (struct CondUBRectangle *) c1.cond;

    INFO("Have to implement this test!");
    CHECK_EQ(true, false);
    ...
    // struct CondUBRectangle *p2 = (struct CondUBRectangle *) c2.cond;
